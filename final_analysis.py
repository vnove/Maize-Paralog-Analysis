#LCR-Paralogy Evolutionary Analysis Pipeline
#This comprehensive Python analysis script (final_analysis.py) investigates the evolutionary relationship between Low Copy Repeats (LCRs) and gene paralogy in maize genomes through statistical correlation analysis and position overlap studies. The pipeline processes two primary datasets: comprehensive_lcr_paralog_dataset.csv containing gene-level LCR counts, paralog counts, and complexity measures, and lcr_analysis_results_Q1_lcr_positions.csv with pairwise gene relationships and positional similarity metrics. The script performs multi-faceted analyses including LCR distribution patterns, paralogy statistics, chromosomal distribution comparisons, and evolutionary distance assessments to determine whether paralogs exhibit related formation (similar LCR positions) versus independent formation (different LCR positions).
#The analysis generates five detailed CSV output files containing enhanced gene-level metrics with evolutionary strategy classifications, comprehensive pairwise relationship data with formation type categorizations, complete statistical summaries, strategy-based comparisons, and subgroup correlation analyses. Through rigorous hypothesis testing using Spearman and Pearson correlations, Mann-Whitney U tests, and Kruskal-Wallis analyses, the script determines whether maize follows compensatory evolution (negative LCR-paralogy correlation, similar to prokaryotes) or complementary evolution (positive correlation, suggesting eukaryotic-specific mechanisms). The pipeline concludes with a comprehensive research report providing biological interpretation, statistical significance assessment, and research implications for understanding genome evolution patterns in plant species.
#!/usr/bin/env python3
"""
